...
async function generateQuestions() {
    ...
    const prompt = `
        ...
        他們3個都是一個2維array, 其中x[i][j]表示在某個介詞在某個慨念維度上的分數（1-5）

        x[0][n], x[1][n], x[2][n], x[3][n] 分別對應 4 個概念維度：
        0.	時間跨度：1分表示極精確的小時段，如具體的時間點，5分表示廣泛的長時段，如月份
        1.	空間範圍：1分表示極精確的位置，如具體地址，5分表示廣大的空間 ，如地點
        2.	動作強度：1分表示靜態事物，如物體所在的位置 ，5分表示動態事物 ，如移動到地點
        3.	對象關連：1分表示直接關係描述，如單純描述所見情況  ，5分表示潛在關係描述 ，如表達行為的原因 

        x[n][0], x[n][1], x[n][2], x[n][3], x[n][4] 分別對應5個介詞：
        0. at
        1. on
        2. in
        3. to
        ...
        以x[0][0]為例，表示學生在「時間跨度」這個概念維度上對「at」這個介詞的理解程度分數（1-5）
